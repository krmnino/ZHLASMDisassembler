...
ErrorCode assemble_SIY(size_t table_index, const char* operands_token, uint8_t* bin_buffer){
...
ErrorCode display_SIY(Instruction* instr){
    if(instr == NULL){
...
ErrorCode disassemble_SIY(size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    char buffer[MAX_OPERANDS_LEN];
    bool i2_unused = INSTRUCTION_TABLE[table_index].unused_operands & I2_UNUSED;
    size_t i = 0;
    // D1:
