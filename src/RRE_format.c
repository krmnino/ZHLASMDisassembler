...
ErrorCode assemble_RRE(Context* c, size_t table_index, const char* operands_token, uint8_t* bin_buffer){
...
ErrorCode display_RRE(Context* c, Instruction* instr){
    if(instr == NULL){
...
ErrorCode disassemble_RRE(Context* c, size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    char buffer[MAX_OPERANDS_LEN];
    bool r1_unused = INSTRUCTION_TABLE[table_index].unused_operands & R1_UNUSED;
    bool r2_unused = INSTRUCTION_TABLE[table_index].unused_operands & R2_UNUSED;
    size_t i = 0;
