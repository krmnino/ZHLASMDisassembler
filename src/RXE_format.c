...
ErrorCode assemble_RXE(Context* c, size_t table_index, const char* operands_token, uint8_t* bin_buffer){
...
ErrorCode display_RXE(Context* c, Instruction* instr){
    if(instr == NULL){
    ...
    printf("+--------+----+----+----+------------+----+----+--------+\n");
    printf("| OPCODE | R1 | X2 | B2 |     D2     | M3 | // | OPCODE |\n");
    printf("+--------+----+----+----+------------+----+----+--------+\n");
    ...
    printf("OPCODE:   %s\n", conv_buffer);
    hex_str_2_char_str((void*)&instr->binary, MAX_INSTRUCTION_LEN, 0, conv_buffer, MAX_PRINTOUT_FIELD_LEN, length * 2, NO_SKIP, false);
    printf("BINARY:   %s\n", conv_buffer);
    printf("LENGTH:   0x%x\n", length);
    printf("FORMAT:   RXE\n");
    printf("OFFSET:   0x%lx\n", instr->offset);
...
ErrorCode disassemble_RXE(Context* c, size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    return OK;
}