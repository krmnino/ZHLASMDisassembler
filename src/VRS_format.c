...
ErrorCode assemble_VRS(size_t table_index, const char* operands_token, uint8_t* bin_buffer){
...
ErrorCode display_VRS(Instruction* instr){
    if(instr == NULL){
...
ErrorCode disassemble_VRS(size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    char buffer[MAX_OPERANDS_LEN];
    InstructionFormat format = INSTRUCTION_TABLE[table_index].format;
    bool m4_unused = INSTRUCTION_TABLE[table_index].unused_operands & M4_UNUSED;
    size_t i = 0;
