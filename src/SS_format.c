...
ErrorCode assemble_SS(size_t table_index, const char* operands_token, uint8_t* bin_buffer){
...
ErrorCode display_SS(Instruction* instr){
    if(instr == NULL){
...
ErrorCode disassemble_SS(size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    char buffer[MAX_OPERANDS_LEN];
    InstructionFormat format = INSTRUCTION_TABLE[table_index].format;
    size_t i = 0;
    switch (format){
