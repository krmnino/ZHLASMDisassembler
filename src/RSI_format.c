...
ErrorCode assemble_RSI(Context* c, size_t table_index, const char* operands_token, uint8_t* bin_buffer){
...
ErrorCode display_RSI(Context* c, Instruction* instr){
    if(instr == NULL){
...
ErrorCode disassemble_RSI(Context* c, size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    return OK;
}