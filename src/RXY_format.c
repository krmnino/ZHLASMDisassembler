...
ErrorCode assemble_RXY(size_t table_index, const char* operands_token, uint8_t* bin_buffer){
        ...
        case D2:
            if(operands_token[i] == ','){
                ...
                if(b_idx >= MAX_5CHR_LEN){
                    ...
                    sprintf((char*)&Context.msg_extras[2], "%d", MAX_5CHR_LEN);
                    return Context.error_code;
                }
                buffer[b_idx] = operands_token[i];
                b_idx++;
        ...
        case X2:
            if(operands_token[i] == ','){
                ...
                if(b_idx >= MAX_1CHR_LEN){
                    Context.error_code = INVALID_OPERAND_LENGTH;
                    sprintf((char*)&Context.msg_extras[0], "%ld", Context.n_line);
                    strcpy((char*)&Context.msg_extras[1], "X2");
                    sprintf((char*)&Context.msg_extras[2], "%d", MAX_1CHR_LEN);
                    return Context.error_code;
                }
                buffer[b_idx] = operands_token[i];
...
ErrorCode display_RXY(Instruction* instr){
    if(instr == NULL){
    ...
    case RXYa:
        printf("+--------+----+----+----+------------+--------+--------+\n");
        printf("| OPCODE | R1 | X2 | B2 |    DL2     |  DH2   | OPCODE |\n");
        printf("+--------+----+----+----+------------+--------+--------+\n");
        printf("0        8    C    10   14           20       28      2F\n");
        break;
    case RXYb:
        printf("+--------+----+----+----+------------+--------+--------+\n");
        printf("| OPCODE | M1 | X2 | B2 |    DL2     |  DH2   | OPCODE |\n");
        printf("+--------+----+----+----+------------+--------+--------+\n");
        printf("0        8    C    10   14           20       28      2F\n");
        break;
    default:
...
ErrorCode disassemble_RXY(size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    char buffer[MAX_OPERANDS_LEN];
    size_t i = 0;
    // R1/M1:
    memset(&buffer, 0, sizeof(buffer));
