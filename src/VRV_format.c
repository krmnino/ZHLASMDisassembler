...
ErrorCode assemble_VRV(size_t table_index, const char* operands_token, uint8_t* bin_buffer){
...
ErrorCode display_VRV(Instruction* instr){
    if(instr == NULL){
...
ErrorCode disassemble_VRV(size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    char buffer[MAX_OPERANDS_LEN];
    bool m3_unused = INSTRUCTION_TABLE[table_index].unused_operands & M3_UNUSED;
    size_t i = 0;
    // V1:
