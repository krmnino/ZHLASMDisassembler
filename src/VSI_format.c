...
ErrorCode assemble_VSI(size_t table_index, const char* operands_token, uint8_t* bin_buffer){
...
ErrorCode display_VSI(Instruction* instr){
    if(instr == NULL){
...
ErrorCode disassemble_VSI(size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    char buffer[MAX_OPERANDS_LEN];
    size_t i = 0;
    // V1:
    memset(&buffer, 0, sizeof(buffer));
