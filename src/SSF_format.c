...
ErrorCode assemble_SSF(Context* c, size_t table_index, const char* operands_token, uint8_t* bin_buffer){
...
ErrorCode display_SSF(Context* c, Instruction* instr){
    if(instr == NULL){
    ...
    printf("+--------+----+----+----+------------+----+------------+\n");
    printf("| OPCODE | R3 | OP | B1 |     D1     | B2 |     D2     |\n");
    printf("+--------+----+----+----+------------+----+------------+\n");
...
ErrorCode disassemble_SSF(Context* c, size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    return OK;
}