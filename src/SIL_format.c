...
ErrorCode assemble_SIL(size_t table_index, const char* operands_token, uint8_t* bin_buffer){
...
ErrorCode display_SIL(Instruction* instr){
    if(instr == NULL){
...
ErrorCode disassemble_SIL(size_t table_index, const uint8_t* bin_buffer, char* operands_token){
    char buffer[MAX_OPERANDS_LEN];
    size_t i = 0;
    // D1:
    memset(&buffer, 0, sizeof(buffer));
